...
class ContextualVectorDB:
    ...
    def __init__(self, name: str = "base_db", use_local_embeddings: bool = None):
        ...
        if self.use_local_embeddings:
            ...
            print("  Using Gemini API embeddings (may hit rate limits)")
            ...
            try:
                if 'content' in item[1]:  # It's a text chunk
                    ...
                    try:
                        ...
                        if "429" in err_str or "RESOURCE_EXHAUSTED" in err_str:
                            ...
                            try:
                                ...
                                print(f"    Retry failed for chunk {idx}: {e2}")
                                contextualized = chunk_text
                        else:
                            print(f"    Context failed for chunk {idx}, using raw: {e}")
                            contextualized = chunk_text
                    ...
                    try:
                        ...
                        print(f"    Image context failed for {image_path}: {e}")
                        contextualized = f"Image from paper: {title}. Path: {image_path}"
                ...
                if result:
                    ...
                    if is_image_fallback:
                        ...
                        if image_fail_streak >= IMAGE_FAIL_THRESHOLD:
                            ...
                            tqdm.write(f"    Image fail #{image_fail_streak}, "
    ...
    def _embed_and_store_optimized(self, texts: List[str], data: List[Dict[str, Any]]):
        ...
        if self.use_local_embeddings and self.local_model:
            ...
            print("   Using rate-limited mode for free tier (15 RPM limit)")
        ...
        for i in range(start_index, len(texts), batch_size):
            ...
            for attempt in range(max_retries):
                ...
                print(f"  Batch {batch_num} failed after {max_retries} retries. Using placeholders.")
        ...
        if failed_indices:
            print(f"  {len(failed_indices)} embeddings failed and use placeholders")
...
class ElasticSearchBM25:
    ...
    def __init__(self, name: str = "contextual_bm25_index", timeout: int = 5):
        ...
        try:
            ...
            print(f"  ES init error: {init_error}")
...
def create_elasticsearch_bm25_index(db: ContextualVectorDB) -> Optional[ElasticSearchBM25]:
    ...
    """
    if not ES_AVAILABLE:
        print(" Elasticsearch not installed. Using vector search only.")
        return None
    ...
    try:
        ...
        print(" Continuing with vector search only (no BM25 hybrid search).")
        return None
    except Exception as e:
