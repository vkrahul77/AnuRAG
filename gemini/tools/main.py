"""
MuaLLM-Gemini: Main Entry Point
Multimodal Large Language Model for Analog Circuit Design using Google Gemini

This module provides the main entry point for the MuaLLM-Gemini system, which can:
...
if not GEMINI_AVAILABLE:
    try:
        ...
        if api_key:
            ...
            GEMINI_AVAILABLE = True
            print("⚠️ Using deprecated google-generativeai package. Consider upgrading to google-genai.")
    except ImportError:
...
def detect_and_fix_repetition(text: str, min_repeat_len: int = 30, max_repeats: int = 2) -> str:
    ...
    if len(paragraphs) > 5:
        ...
        if repeated_paras:
            ...
            if len(cleaned_paras) < len(paragraphs):
                print("⚠️ Detected paragraph repetition - cleaning output")
                text = '\n\n'.join(cleaned_paras)
    ...
    if len(lines) > 10:
        ...
        if repeated_lines:
            ...
            if len(cleaned_lines) < len(lines):
                print("⚠️ Detected line repetition - cleaning output")
                text = '\n'.join(cleaned_lines)
            ...
            if count > max_repeats:
                ...
                if first_occurrence > 0:
                    ...
                    if truncate_at < len(text) - 200:
                        print("⚠️ Detected phrase repetition loop - truncating output")
                        text = text[:truncate_at] + "\n\n[... repetitive output truncated ...]"
                        break
    
    if len(text) < original_len:
        removed = original_len - len(text)
        print(f"✂️ Removed {removed} characters of repetitive content")
    ...
    if len(cleaned_lines) < len(lines):
        text = '\n'.join(cleaned_lines)
        print(f"✂️ Cleaned {len(lines) - len(cleaned_lines)} corrupted/duplicate lines")
                ...
                if code_blocks:
                    ...
                    if len(remaining) > 500:
                        ...
                        if remaining.count(sample[:50]) > 1:
                            ...
                            print("✂️ Truncated repetitive content after code block")
...
def extract_images_from_query(query: str) -> tuple[str, List[str]]:
    ...
    if image_paths:
        ...
        print("ℹ️  No image paths extracted from query")
        ...
        for attempt in range(max_retries):
            try:
                ...
                if is_retryable and attempt < max_retries - 1:
                    ...
                    print(f"⚠️ Server error (attempt {attempt + 1}/{max_retries}): {str(e)[:100]}")
...
def query(question: str, max_turns: int = 10, system_prompt: str = None, existing_bot: 'Agent' = None) -> Dict[str, Any]:
    ...
    while i < max_turns:
        ...
        if result is None:
            print("⚠️ Model returned empty response, retrying...")
            continue
        ...
        if "Action:" not in result:
            ...
            if i == 1 and last_observation is None and needs_paper_search and not answer_is_substantial:
                print("\n⚠️ Agent tried to answer without searching. Reminding to use database...")
...
Original question: """ + question
        ...
        if actions:
            ...
            try:
                ...
                print("ℹ️ Falling back to LLM internal knowledge (no RAG search)")
        ...
        try:
            ...
            if current_mode == 1:
                ...
                try:
                    ...
                    print(f"⚠️ Could not introspect LUT files ({e}), using static reference")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='MuaLLM-Gemini: Multimodal AI for Analog Circuit Design',
